...
#include <sqlite3.h>
SQLiteStatement::SQLiteStatement(sqlite3* db, std::string cmd)
{
...
int SQLiteStatement::Size()
{
    return nbreturn;
}


int SQLiteStatement::GetInt(std::string pkey, int id)
{
    int ri=-1;
...
std::string SQLiteStatement::GetString(std::string pkey, int id)
{
    std::string lstr;
...
float SQLiteStatement::GetFloat(std::string pkey, int id)
{
    float rf=-1;
...
int SQLiteManager::_exec(std::string cmd,char** perr)
{
    int ir = sqlite3_exec(db,cmd.c_str(),0,0,perr);
    return ir;
}

int SQLiteManager::_exec_call(std::string cmd,int (*fcall)(void* ptr,int argc,char** sarg,char** colname),char** perr)
{
    int ir = sqlite3_exec(db,cmd.c_str(),fcall,0,perr);
    return ir;
}

SQLiteManager::SQLiteManager(std::string path)
{
    int rc = sqlite3_open(path.c_str(),&db);
    if(rc)
        // erreur d'ouverture
    {
        puts("SQLiteManager : erreur db");
    }
    else
        // db ouverte
    {
        puts("SQLiteManager : open db");
    }
}

void SQLiteManager::CreateStatement(std::string name, std::string rqt)
{
    SQLiteStatement* stmt = new SQLiteStatement(db,rqt);
    lstmt.insert({name, stmt});
}

SQLiteStatement* SQLiteManager::LoadStatement(std::string name)
{
    if(!lstmt.empty())
    {
...
void SQLiteManager::ClearStatement()
{
    lstmt.clear();
}
